...
tabs = st.tabs(["🥔 Potato Model", "🍅 Tomato Model"])
...
with tabs[0]:
    ...
    with c1:
        ...
        report_path = os.path.join(REPORTS_DIR, "potato_report.csv")
    ...
    with c2:
        ...
        cm_path = os.path.join(CONF_MATRIX_DIR, "potato_confmat.png")
        if os.path.exists(cm_path):
            img = Image.open(cm_path)
            st.image(img, caption="Potato Confusion Matrix (Augmentation + SE Attention)", use_container_width=True)
        else:
...
with tabs[1]:
    ...
    # Grid of key indicators
    m1, m2, m3, m4 = st.columns(4)
    m1.markdown('<div class="metric-card"><div class="metric-val">78.9%</div><div class="metric-lbl">Accuracy (BR→BR)</div></div>', unsafe_allow_html=True)
    ...
    with c2:
        ...
        if os.path.exists(cm_path):
            img = Image.open(cm_path)
            st.image(img, caption="Tomato Confusion Matrix (BR→BR, Aligned Deployment)", use_container_width=True)
        else:
            st.warning("Confusion matrix image not found.")
    st.markdown("</div>", unsafe_allow_html=True)